/**
 * Errors that are safe to show a user. Anything else is reported as a generic
 * "Something went wrong" so stack traces never leak.
 *
 * Shared, because the rules that raise them are shared: the same program
 * generator runs on the server and on the device, and both need to say the
 * same thing when a selection is invalid. `statusCode` is how the API maps one
 * onto HTTP; on-device it is simply ignored.
 */
