...
describe('the neck floor', () => {
  test('accepts the necks adult women actually have', () => {
    // This floor was 35 cm, which is above the average adult female neck of
    // 30-34 cm. Since the neck is required to finish the assessment, that
    // locked a large share of women out of the thing they had just paid for.
    for (const cm of [28, 30, 32, 34]) {
      assert.ok(accepts('neckCm', cm), `${cm} cm should be accepted`);
    }
  });

  test('and still refuses a reading that cannot be a neck', () => {
    ...
    assert.ok(!accepts('neckCm', 3));
    assert.ok(!accepts('neckCm', 25.9));
    assert.ok(accepts('neckCm', 26));
  });
